...
module.exports = {
	...
	order.status.toLowerCase() === 'pennding'
		...
		: ''
			...
			Order Status: <b style = "color: ${
	this.statusColors[order.status.toLowerCase()]
}">${Filters.CapitalizeFirst(order.status)}</b><br>
			Delivery Address:
			<small>${order.delivery_address.building}, ${order.delivery_address.street}, ${
	order.delivery_address.text_data.area
}, ${order.delivery_address.text_data.city}</small><br>
			Orderd Items:<br>
			${this.getItems(order.cart.items)}
			<br>
			<a style = "text-decoration: none; color: #2a0d45" href = "${
	cogs.website
}/profile/orders/${
	order.order_no
}"> Check it out <b style = "color: #2196f3">here!</b> </a><br>
