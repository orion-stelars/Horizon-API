...
module.exports = {
	...
	async register(req, res) {
		...
		const appSpecs = await Deskaf.find({})
		if (appSpecs && appSpecs.length)
			return res.status(400).send('You Cant Add Multiple Configurations.');
	...
	async update(req, res) {
		...
		if (!appSpecs || !appSpecs.length)
			return res.status(404).send('Deskaf Not Found.');
		res.send(appSpecs[0])
	},
			...
			fs.access(getFilePath(appSpecs[0].background), fs.F_OK, err => {
				...
				return res
					.set({
						...
						"Content-Disposition": `inline; filename="designkaf${path.extname(
							appSpecs[0].background
						)}"`
					})
