...
const generateFileName = (req, file, data) => {
	return [data, path.extname(file.originalname)].join('');
}
...
module.exports = {
    ...
    res.header('Authorization', token).send({
			token,
      ..._.pick(user, ['_id', 'name', 'email', 'roles', 'avatar', 'points'])
    })
  },
      ...
      if (req.body.roles && decoded.roles.indexOf('admin') !== -1) {
        ...
        if (req.body.blocked !== undefined) {
          ...
          if (typeof updates.is_verified.email === 'string') {
              ...
              updates.is_verified.verified == 'true';
					}
    ...
    const user = await User.findOneAndUpdate({ _id: decoded._id }, updates, {
			new: true
		})
    if (!user) return res.status(404).send('User Not Found.');
		return res.send(user)
		...
		if (user.avatar) {
			return res
				.set({
					...
					"Content-Disposition": `inline; filename="${user.name}${path.extname(
						user.avatar
					)}"`
				})
